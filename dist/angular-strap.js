...
angular.module('mgcrea.ngStrap.datepicker', ['mgcrea.ngStrap.helpers.dateParser', 'mgcrea.ngStrap.tooltip'])
  ...
  .provider('$datepicker', function() {
    ...
    this.$get = ["$window", "$document", "$rootScope", "$sce", "$dateUtil", "datepickerViews", "$tooltip", function($window, $document, $rootScope, $sce, $dateUtil, datepickerViews, $tooltip) {

      var bodyEl = angular.element($window.document.body);
      var isNative = /(ip(a|o)d|iphone|android)/ig.test($window.navigator.userAgent);
      var isTouch = ('createTouch' in $window.document) && isNative;
      if(!defaults.lang) defaults.lang = $dateUtil.getDefaultLocale();
  ...
  .directive('bsDatepicker', ["$window", "$parse", "$q", "$dateUtil", "$datepicker", "$timeout", function($window, $parse, $q, $dateUtil, $datepicker, $timeout) {
    ...
    return {
      ...
      link: function postLink(scope, element, attr, controller) {
        ...
        var formatDate = function(date, format) {
          return $dateUtil.formatDate(date, format, lang);
        };
        ...
        var dateParser = $dateUtil.getDateParser({format: options.dateFormat, lang: lang, strict: options.strictFormat});
        ...
        controller.$render = function() {
          ...
          element.val(!controller.$dateValue || isNaN(controller.$dateValue.getTime()) ? '' : formatDate(controller.$dateValue, options.dateFormat));
        };
  ...
  .provider('datepickerViews', function() {
    ...
    this.$get = ["$dateUtil", "$sce", function($dateUtil, $sce) {
      ...
      return function(picker) {
        ...
        var weekDaysMin = $dateUtil.weekdaysShort(lang);
...
angular.module('mgcrea.ngStrap.timepicker', ['mgcrea.ngStrap.helpers.dateParser', 'mgcrea.ngStrap.tooltip'])
  ...
  .provider('$timepicker', function() {
    ...
    this.$get = ["$window", "$document", "$rootScope", "$sce", "$dateUtil", "$tooltip", function($window, $document, $rootScope, $sce, $dateUtil, $tooltip) {
      ...
      function timepickerFactory(element, controller, config) {
        ...
        var format = $dateUtil.getDatetimeFormat(options.timeFormat, lang);

        var hoursFormat = $dateUtil.hoursFormat(format),
          timeSeparator = $dateUtil.timeSeparator(format),
          minutesFormat = $dateUtil.minutesFormat(format),
          showAM = $dateUtil.showAM(format);
  ...
  .directive('bsTimepicker', ["$window", "$parse", "$q", "$dateUtil", "$timepicker", "$timeout", function($window, $parse, $q, $dateUtil, $timepicker, $timeout) {
    ...
    return {
      ...
      link: function postLink(scope, element, attr, controller) {
        ...
        // Initialize parser
        var dateParser = $dateUtil.getDateParser({format: options.timeFormat, lang: lang});
        ...
        controller.$render = function() {
          // console.warn('$render("%s"): viewValue=%o', element.attr('ng-model'), controller.$viewValue);
          element.val(!controller.$dateValue || isNaN(controller.$dateValue.getTime()) ? '' : formatDate(controller.$dateValue, options.timeFormat));
        };
